...
xmlhttp.open("GET","leave.php?cdate="+cdate+"&date2="+date2,true);
xmlhttp.send();
}
</script>
...
?>
 ...
 <tr>
 <td height="33" colspan="5">Date From 
   <input size="15" id="f_date1" name="f_date1" value="<?php echo @$_POST['f_date1'];?>" />
   <button id="f_btn1">...</button>   <script type="text/javascript">//<![CDATA[
      Calendar.setup({
        inputField : "f_date1",
        trigger    : "f_btn1",
    ...
    //]]></script>	 <input name="submit" type="button" value="Show Report" onClick="showcont();" /></td>
  </tr>
 <tr>
    ...
    while($row = mysql_fetch_array($tree))
            {
               ...
               {  
                ?>
						<li style="background-color:#EEF5FD"><a href="dist_attend.php" ><?php echo $rowdist['disname']; ?></a>
                            <ul>
							<?php
							$disid=$rowdist['disid'];
...
</div>	</td> <td width="593" valign="top"><div id="hosid"></div>	</td></tr>
	</table>	
</form>
